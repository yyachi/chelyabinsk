...
#'
#' @param reflame A pmlame of reference with row of stone such as
...
cbk.lame.acqify <- function(reflame,acqlist,stonify_regexp="@[[:digit:]]+$",verbose=FALSE) {
  acqlist1           <- gsub("-","_",acqlist)
  stonelist1         <- gsub(stonify_regexp,"",acqlist1)

  rownames(reflame)  <- gsub("-","_",rownames(reflame))
  pmlame             <- reflame[stonelist1,]
  rownames(pmlame)   <- acqlist
