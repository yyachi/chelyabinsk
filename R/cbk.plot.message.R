#' Display a diagram with message.  When show error massage, quote in
#' advance.
#' 
...
cbk.plot.message <- function(pmlfile_or_stone,msg) {
  XX    <- 1:100
  ## YY <- rep(1,100)
  YY1    <- 5*1.02^XX*sin(XX/3)        + 20
  YY2    <- 5*1.02^XX*sin(XX/3+3.14/2) + 20
  YY3    <- 5*1.02^XX*sin(XX/3+3.14)   + 20
  mylim <- c(0,100)
  ...
  text(3,97,wrap_line(msg,76),adj=c(0,1))
