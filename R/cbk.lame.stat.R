...
#' @param groupfy_regexp A regexp string to groupfy stones.
...
cbk.lame.stat <- function(pmlame,groupfy_regexp="@[[:digit:]]+$",verbose=FALSE){
  meanlame0 <- cbk.lame.regulate(pmlame,mean=T,error=F,extra=F)
  meanlame  <- cbk.lame.colMeans(meanlame0,groupfy_regexp=groupfy_regexp)
  errorlame <- cbk.lame.colSds(meanlame0,groupfy_regexp=groupfy_regexp)
