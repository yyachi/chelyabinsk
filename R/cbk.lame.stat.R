...
cbk.lame.stat <- function(pmlame,stonify_regexp="@[[:digit:]]+$",verbose=FALSE){
  meanlame0 <- cbk.lame.regulate(pmlame,mean=T,error=F,extra=F)
  meanlame  <- cbk.lame.colMeans(meanlame0,stonify_regexp=stonify_regexp)
  errorlame <- cbk.lame.colSds(meanlame0,stonify_regexp=stonify_regexp)
